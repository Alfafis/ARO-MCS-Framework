...
// === TESTE 3 — RB-03: pedir menos que MIN_ITERATIONS ainda roda o piso ===
{
  ...
  assert.ok(
    r.iterationsRun >= MIN_ITERATIONS,
    `iterationsRun (${r.iterationsRun}) deve ser >= MIN_ITERATIONS (${MIN_ITERATIONS})`
  )
...
// === TESTE 6 — Engine 6: categoria de maior variância domina o ranking de Risk Drivers ===
{
  ...
  console.log(
    `✔ Risk Drivers: ${r.riskDrivers[0].name} (r=${r.riskDrivers[0].correlation.toFixed(3)}) > ${r.riskDrivers[1].name} (r=${r.riskDrivers[1].correlation.toFixed(3)})`
  )
}
...
// === TESTE 7 — §9: cenários são monotônicos (Otimista ≤ Moderado ≤ Pessimista ≤ Estresse) ===
{
  ...
  assert.ok(
    otimista <= moderado && moderado <= pessimista && pessimista <= estresse,
    `cenários devem ser monotônicos: ${otimista} <= ${moderado} <= ${pessimista} <= ${estresse}`
  )
  console.log(
    `✔ Cenários monotônicos: Otimista=${otimista.toFixed(0)} Moderado=${moderado.toFixed(0)} Pessimista=${pessimista.toFixed(0)} Estresse=${estresse.toFixed(0)}`
  )
}
...
// === TESTE 8 — Engine 5: calibração escolhe o nível de risco certo pelo CV ===
{
  ...
  assert.equal(calLow.margemSeguranca, 0.1)
  assert.equal(calLow.provisaoBase, rLow.p50)
  assert.ok(Math.abs(calLow.provisaoFinal - calLow.provisaoBase * 1.1) < 1e-6)
  ...
  assert.equal(calHigh.margemSeguranca, 0.2)
  assert.equal(calHigh.provisaoBase, Math.max(rHigh.p95, rHigh.cvar95))

  console.log(
    `✔ Calibração: faixa estreita→${calLow.nivelRisco} (${(calLow.margemSeguranca * 100).toFixed(0)}%), faixa larga→${calHigh.nivelRisco} (${(calHigh.margemSeguranca * 100).toFixed(0)}%)`
  )
}
