...
export function computeMonetaryMethods(filteredBase: number): MonetaryMethod[] {
  const [simple, compound, inflation, ipca] = computeMonetaryValues(filteredBase)
  return [
    { label: 'Juros simples — 10,75%/ano',              value: fmtBRL(simple)    },
    { label: 'Juros compostos — 10,75%/ano',            value: fmtBRL(compound)  },
    { label: 'Inflação constante — 3,4%/ano',           value: fmtBRL(inflation) },
    { label: 'Escalonamento — IPCA variável 2024–2033', value: fmtBRL(ipca)      },
  ]
}
