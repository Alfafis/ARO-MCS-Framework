...
export const custoUnitMedio = (item: ItemRemediacao): number => (item.custoUnitMin + item.custoUnitMax) / 2

// Custo total por item = quantidade × custo unitário médio.
export const custoTotalItem = (item: ItemRemediacao): number => item.quantidade * custoUnitMedio(item)
