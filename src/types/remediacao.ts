...
export function mapCategoriaRemediacaoRow(row: CategoriaRemediacaoRow, items: ItemRemediacao[] = []): CategoriaRemediacao {
  return {
    id:        row.id,
...
export const custoUnitMedio = (item: ItemRemediacao): number =>
  (item.custoUnitMin + item.custoUnitMax) / 2

// Custo total por item = quantidade × custo unitário médio.
export const custoTotalItem = (item: ItemRemediacao): number =>
  item.quantidade * custoUnitMedio(item)

// Total da categoria = soma dos custos totais dos itens.
export const custoTotalCategoria = (cat: CategoriaRemediacao): number =>
  cat.items.reduce((acc, i) => acc + custoTotalItem(i), 0)

// Total geral do módulo — soma das categorias.
export const custoTotalRemediacao = (categorias: CategoriaRemediacao[]): number =>
