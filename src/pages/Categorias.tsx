    ...
    <div className="flex flex-col h-full">
      <header className="flex items-start justify-between px-4 sm:px-8 py-4 sm:py-[22px] gap-4 shrink-0">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Button
            variant="primary"
            onClick={() => addCategoria(projeto.id).catch(() => showToast('Não foi possível criar a categoria.'))}
          >
            {t.createCategoryBtn}
          </Button>
        </div>
                ...
                <div className="flex flex-wrap items-center justify-center gap-2">
                  {TIPOS_COM_EXEMPLO.map((tp) => (
                    <Button
                      ...
                      variant="ghost"
                      onClick={() =>
                        carregarTemplateExemplo(projeto.id, tp.id).catch(() =>
                          showToast('Não foi possível carregar o exemplo.')
                        )
                      }
                    >
              ...
              return (
                <CategoryBlock
                  ...
                  onChange={(field, value) =>
                    updateCategoria(projeto.id, cat.id, field, value).catch(() => showToast('Não foi possível salvar.'))
                  }
                  onRename={(novoNome) => {
                  ...
                  onRemoveItem={(itemId) =>
                    removeItem(projeto.id, cat.id, itemId).catch(() => showToast('Não foi possível remover o item.'))
                  }
                  onUpdateItem={(itemId, field, value) => updateItem(projeto.id, cat.id, itemId, field, value)}
                  onSaveItem={(itemId, field, value) =>
                    saveItem(itemId, field, value).catch(() => showToast('Não foi possível salvar o item.'))
                  }
                  horizonYears={projeto.horizonteAnos}
                  onSaveCustoProvavel={(valor) =>
                    updateCategoriaCustoProvavel(projeto.id, cat.id, valor).catch(() =>
                      showToast('Não foi possível salvar o custo provável.')
                    )
                  }
                  onSaveDesembolso={(itemId, valores) =>
                    updateItemDesembolso(projeto.id, cat.id, itemId, valores).catch(() =>
                      showToast('Não foi possível salvar o desembolso por ano.')
                    )
                  }
                  onAddCampoOpProjeto={() =>
                    addCampoOp(projeto.id, cat.id).catch(() => showToast('Não foi possível criar o campo operacional.'))
                  }
                  onRemoveCampoOpProjeto={(campoId) =>
                    removeCampoOp(projeto.id, cat.id, campoId).catch(() =>
                      showToast('Não foi possível remover o campo operacional.')
                    )
                  }
                  onUpdateCampoOpProjeto={(campoId, field, value) =>
                    updateCampoOp(projeto.id, cat.id, campoId, field, value)
                  }
                  onSaveCampoOpProjeto={(campoId, field, value) =>
                    saveCampoOp(campoId, field, value).catch(() =>
                      showToast('Não foi possível salvar o campo operacional.')
                    )
                  }
                />
              )
