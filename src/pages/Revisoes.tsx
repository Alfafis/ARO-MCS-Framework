  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        ...
        actions={
          <Button variant="primary" onClick={handleNovaRevisao}>
            {t.newRevision}
          </Button>
        }
      />
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-3">
                        <span
                          className={`inline-flex items-center justify-center w-[34px] h-[34px] rounded-[10px] font-mono font-bold text-[13px] flex-none ${badgeCls}`}
                        >
                          {rev.codigo}
                        </span>
                        <div>
                      ...
                      <div className="flex items-center gap-3 flex-none">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[12px] font-semibold ${meta.cls}`}
                        >
                          {meta.label}
                        </span>
                        {rev.hash && <Button variant="ghost">{t.viewPdf}</Button>}
                      </div>
                    </div>
                        ...
                        {isEditing && (
                          <div className="mt-3 flex flex-col gap-2">
                            ...
                            {projeto.remediacaoHabilitada && (
                              <label className="flex items-start gap-2 mt-1 cursor-pointer group">
                                <input
                                  ...
                                  checked={
                                    (rev as unknown as { incluir_remediacao?: boolean }).incluir_remediacao ?? false
                                  }
                                  onChange={async (e) => {
                                    const incluir = e.target.checked
                                    setRevisoes((prev) =>
                                      prev.map((r) =>
                                        r.id === rev.id ? ({ ...r, incluir_remediacao: incluir } as RevisaoRow) : r
                                      )
                                    )
                                    await supabase
                                      .from('revisoes')
                                      .update({ incluir_remediacao: incluir })
                                      .eq('id', rev.id)
                                  }}
                                />
                            ...
                            <div className="flex gap-2">
                              <Button variant="ghost" onClick={() => handleSalvar(rev.id)}>
                                {t.saveChanges}
                              </Button>
                              <Button variant="primary" onClick={() => handlePublicar(rev.id)}>
                                {t.publishRevision}
                              </Button>
                            </div>
                          </div>
