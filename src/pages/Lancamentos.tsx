...
export default function Lancamentos() {
  ...
  const filtered = rows.filter(
    (r) => r.categoria.toLowerCase().includes(search.toLowerCase()) && (filter === 'all' || r.status === filter)
  )
  ...
  const confirmAdd = useCallback(
    async (form: { categoria: string; periodo: string; valor: string }) => {
          ...
          <div className="min-w-[540px]">
            <div className="plist-head">
              {[t.colCategory, t.colPeriod, t.colValue, t.colStatus, ''].map((col, i) => (
                <span key={i} className={`plist-col-label${i === 2 ? ' right' : ''}`}>
                  {col}
                </span>
              ))}
            </div>
