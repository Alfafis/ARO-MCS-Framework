...
function computeResult(
  ...
  confidence: number
): Omit<SimResult, 'status' | 'iterations' | 'distribution'> {
          ...
          <div className="flex flex-col gap-4">
            {categoryParams.length === 0 && (
              <div className="card text-[0.8125rem] text-c-text-2">
                Este projeto ainda não tem categorias de custo cadastradas — sem itens com custo min/max, não há o que
                simular. Cadastre em "Categorias de custo".
              </div>
            )}
            ...
            {hasRun && (
              <div className="card flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[13px] font-semibold text-c-text leading-tight">
                      Simulação aplicada ao relatório
                    </p>
                    <p className="text-[12px] text-c-text-2 mt-0.5">
                      Visualize o resultado do ponto de vista do cliente.
                    </p>
                  </div>
                </div>
          ...
          <section className="flex flex-col gap-3 pt-2 mt-2 border-t border-[rgba(20,21,26,.08)]">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[12.5px] text-c-text-2 leading-snug max-w-[720px]">
                Aro Simulação <strong>independente</strong> sobre o valor do Ano 10 — roda automaticamente com base nas
                categorias cadastradas, sem depender do botão "Rodar simulação" acima. Mede a incerteza da <em>taxa</em>{' '}
                de escalação futura, complementar à incerteza de <em>escopo</em> da Aro Simulação principal.
              </p>
            </div>
            <SensibilidadeAno10Card baseAno10={sensBase.baseAno10} modoIpcaDisponivel={sensBase.modoIpcaDisponivel} />
          </section>
        )}
      </div>

      {historyOpen && (
        <HistoryModal history={history} onSelect={loadHistoryRun} onClose={() => setHistoryOpen(false)} />
      )}
    </div>
  )
}
