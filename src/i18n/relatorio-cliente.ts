...
export const relatorioClienteT: Record<
  Lang,
  {
    // Header
    portalPill: string
...
> = {
  'pt-BR': {
    ...
    reportNotFoundBody:
      'O link usado não corresponde a nenhum projeto ativo. Verifique o link ou peça um novo à sua consultoria.',
    kpiAvgCost: 'Custo médio',
    simPendingSub: 'Sem simulação rodada ainda',
    kpiAvgCostSubSim: (status) => `Aro Simulação · ${status}`,
    kpiMinMaxRange: 'Faixa otimista–pessimista',
    ...
    modalCodePlaceholder: 'Ex: ABC-2024',
  ...
  en: {
    ...
    reportNotFoundBody:
      'This link does not match any active project. Check the link or ask your consultant for a new one.',
    kpiAvgCost: 'Average cost',
    simPendingSub: 'No simulation run yet',
    kpiAvgCostSubSim: (status) => `Aro Simulação · ${status}`,
    kpiMinMaxRange: 'Optimistic–pessimistic range',
    ...
    modalCodePlaceholder: 'e.g. ABC-2024',
  ...
  es: {
    ...
    reportNotFoundBody:
      'El enlace usado no corresponde a ningún proyecto activo. Verifique el enlace o pida uno nuevo a su consultoría.',
    kpiAvgCost: 'Costo promedio',
    simPendingSub: 'Sin simulación ejecutada aún',
    kpiAvgCostSubSim: (status) => `Aro Simulação · ${status}`,
    kpiMinMaxRange: 'Rango optimista–pesimista',
    kpiMinMaxSubIC: (conf, ic) => `IC ${conf}%: ${ic}`,
    ...
    modalCodePlaceholder: 'Ej: ABC-2024',
