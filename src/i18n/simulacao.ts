...
export const simulacaoT: Record<
  Lang,
  {
    headerTitle: string
...
> = {
  'pt-BR': {
    ...
    simDesc: (n) =>
      `A simulação gera ${n} cenários aleatórios combinando os custos mín/máx de cada item e apresenta a distribuição de probabilidade do custo total.`,
    running: 'Simulando…',
    run: 'Rodar simulação',
    ...
    sensAno10FooterNote:
      'Cada iteração sorteia uma taxa aleatória inteira dentro da faixa configurada e aplica o multiplicador (1 + taxa) sobre o valor do Ano 10 já corrigido por IPCA acumulado (ou provisão, se IPCA incompleto). Mede a incerteza de taxa final acima da incerteza de escopo já capturada pela Aro Simulação principal.',
    seedFooter: (seed, converged) =>
      `Seed ${seed}${converged ? ' · convergiu antes do teto de iterações' : ' · não convergiu — atingiu o teto de iterações'} — mesma seed reproduz este resultado.`,
    calibrationTitle: 'Calibração automática de provisão',
    calibrationHint:
      'Recomendação do método a partir do Coeficiente de Variação (CV) — leitura adicional, não substitui os modos manuais acima.',
  ...
  en: {
    ...
    simDesc: (n) =>
      `The simulation generates ${n} random scenarios combining the min/max costs of each item and presents the probability distribution of the total cost.`,
    running: 'Running…',
    run: 'Run simulation',
    ...
    seedFooter: (seed, converged) =>
      `Seed ${seed}${converged ? ' · converged before the iteration ceiling' : ' · did not converge — hit the iteration ceiling'} — the same seed reproduces this result.`,
    calibrationTitle: 'Automatic provision calibration',
    calibrationHint:
      'Method recommendation from the Coefficient of Variation (CV) — an additional read, does not replace the manual modes above.',
  ...
  es: {
    ...
    simDesc: (n) =>
      `La simulación genera ${n} escenarios aleatorios combinando los costos mín/máx de cada ítem y presenta la distribución de probabilidad del costo total.`,
    running: 'Simulando…',
