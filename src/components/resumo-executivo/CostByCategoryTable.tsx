  ...
  return (
    <div className="card col-span-7">
      <div className="flex items-center gap-1.5 mb-4">
        ...
        <span className="font-semibold text-[0.875rem] text-c-text">Custo por categoria — 8 setores</span>
      </div>

      <table className="w-full border-collapse">
        <thead>
          <tr>
            {(['#', 'Categoria', 'Min', 'Max', 'Atualiz. 2023'] as const).map(col => (
              <th
                key={col}
                className="text-[11px] font-semibold tracking-widest uppercase text-c-text-2 pb-2.5 border-b border-c-line"
                style={{ textAlign: col === '#' || col === 'Categoria' ? 'left' : 'right' }}
              >
                {col}
              </th>
            ))}
        ...
        <tbody>
          ...
          <tr>
            ...
            <td className="pt-3 border-t-2 border-c-line text-[0.8125rem] font-bold text-c-text">Total geral</td>
