...
export default function CltRow({
  ...
  return (
    <div
      ...
      onClick={onOpen}
    >
      {clienteNome && <span className="text-[0.875rem] text-c-text-2 truncate">{clienteNome}</span>}
      ...
      {/* Status */}
      <span
        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[12px] font-semibold justify-self-start ${status.cls}`}
      >
        {status.label}
      </span>
